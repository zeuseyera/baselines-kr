...
class Model( object ):
    ...
    def __init__( self
        ...
        def train(obs, states, rewards, masks, actions, values):
            ...
            policy_loss, value_loss, policy_entropy, _ = sess.run( [pg_loss, vf_loss, entropy, train_op]
                                                                , td_map )
...
def learn( network
    ...
    for update in range(1, total_timesteps//nbatch+1):
        ...
        if update % log_interval == 0 or update == 1:
            ...
            logger.record_tabular("갱신횟수", update)
            logger.record_tabular("총 탐사보수", update*nbatch)
            logger.record_tabular("fps", fps)
            logger.record_tabular("policy_entropy", float(policy_entropy))
            logger.record_tabular("policy_loss", float(policy_loss))
            logger.record_tabular("value_loss", float(value_loss))
            logger.record_tabular("explained_분산", float(ev))
            logger.record_tabular("토막 포상평균", safemean([epinfo['r'] for epinfo in epinfobuf]))
            logger.record_tabular("토막 길이평균", safemean([epinfo['l'] for epinfo in epinfobuf]))
            logger.dump_tabular()
